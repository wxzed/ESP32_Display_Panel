...
/**
 * @brief LCD driver class for NODSICONF display controller
 *
 */
class LCD_NODSICONF: public LCD {
public:
    /**
     * @brief Default basic attributes for NODSICONF LCD controller
     */
     ...
     */
    LCD_NODSICONF(Bus *bus, int width, int height, int color_bits, int rst_io):
     ...
     */
    LCD_NODSICONF(Bus *bus, const Config &config):
     ...
     * @note Vendor initialization commands may vary between manufacturers. Consult LCD supplier for them
     */
    LCD_NODSICONF(const BusFactory::Config &bus_config, const Config &lcd_config):
     ...
     */
    [[deprecated("Use other constructor instead")]]
    LCD_NODSICONF(Bus *bus, uint8_t color_bits = 0, int rst_io = -1): LCD_NODSICONF(bus, 0, 0, color_bits, rst_io)
    {
    }
...
/**
 * @brief Alias for backward compatibility
 * @deprecated Use `esp_panel::drivers::LCD_NODSICONF` instead
 */
using ESP_PanelLcd_NODSICONF [[deprecated("Use `esp_panel::drivers::LCD_NODSICONF` instead")]] =
    esp_panel::drivers::LCD_NODSICONF;
