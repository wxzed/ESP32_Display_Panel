...
/**
 * @brief LCD driver class for SIMPLE display controller
 * 
 ...
 *
 */
class LCD_SIMPLE: public LCD {
public:
    /**
     * @brief Default basic attributes for SIMPLE LCD controller
     */
     ...
     */
    LCD_SIMPLE(Bus *bus, int width, int height, int color_bits, int rst_io):
     ...
     * @note This driver does not send any initialization commands, only display data
     */
    LCD_SIMPLE(Bus *bus, const Config &config):
     ...
     * @note This driver does not send any initialization commands, only display data
     */
    LCD_SIMPLE(const BusFactory::Config &bus_config, const Config &lcd_config):
     ...
     * @note Creates panel handle by calling `esp_lcd_new_panel_simple()` internally
     * @note This driver skips initialization commands and only sets up the panel for data transfer
     */
    bool init() override;
     ...
     */
    [[deprecated("Use other constructor instead")]]
    LCD_SIMPLE(Bus *bus, uint8_t color_bits = 0, int rst_io = -1): LCD_SIMPLE(bus, 0, 0, color_bits, rst_io)
    {
    }
 ...
 * @brief Alias for backward compatibility
 */
using LCDSimple = esp_panel::drivers::LCD_SIMPLE;
